...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
			...
			if (pid.pgcateg() == BTreePageId.LEAF)
				...
				if(f == null)
					return findLeafPage(tid, dirtypages, internalPage.iterator().next().getLeftChild(), perm, f);
    ...
    public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        // System.out.println("SPITTING LEAF PAGE " + page.getId());
        // System.out.println("Before splitting: page id = " + page.getId() + ", num tuples = " + page.getNumTuples());
        ...
        // System.out.println("Middle index = " + midIndex + ", key of middle tuple = " + mid.getField(keyField));
        // copy up into the parent page

        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
        // System.out.println("Parent page id = " + parent.getId()+ ", num tuples = " + parent.getNumEntries()+ ", max entries = "+parent.getMaxEntries());
        ...
        // System.out.println("After splitting: original page id = " + page.getId() + ", num tuples = " + page.getNumTuples());
        // System.out.println("After splitting: new page id = " + newPage.getId() + ", num tuples = " + newPage.getNumTuples());
        ...
        if(page.getRightSiblingId() != null) {
			BTreeLeafPage oldNextPage = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
			oldNextPage.setLeftSiblingId(newPage.getId());
		}
        newPage.setLeftSiblingId(page.getId());
        newPage.setRightSiblingId(page.getRightSiblingId());
        page.setRightSiblingId(newPage.getId());

        // System.out.println("After updating sibling pointers: new page's left sibling = " + newPage.getLeftSiblingId() + ", right sibling = " + newPage.getRightSiblingId());
        // System.out.println("After updating sibling pointers: original page's right sibling = " + page.getRightSiblingId());
        // update dirty pages
        getPage(tid, dirtypages, page.getId(), Permissions.READ_WRITE);
        getPage(tid, dirtypages, newPage.getId(), Permissions.READ_WRITE);
        getPage(tid, dirtypages, parent.getId(), Permissions.READ_WRITE);

        // return the page
        if (field.compare(Op.LESS_THAN, mid.getField(keyField))) {
            return page;
        } else {
            return newPage;
        }
    ...
    public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        // System.out.println("SPLIT INTERNAL PAGE");
        // System.out.println("Before splitting: page id = " + page.getId() + ", num entries = " + page.getNumEntries());
        ...
        // } else {
            ...
            // System.out.println("Middle index = " + midIndex + ", key of middle entry = " + mid.getKey());
            
            page.deleteKeyAndRightChild(mid);
            // push the middle key up into the parent page
            BTreeEntry midEntry = new BTreeEntry(mid.getKey(), page.getId(), newPage.getId());
            BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), mid.getKey());
            ...
            // System.out.println("After splitting: original page id = " + page.getId() + ", num entries = " + page.getNumEntries());
    ...
    public List<Page> insertTuple(TransactionId tid, Tuple t)
        ...
        if (leafPage.getNumEmptySlots() == 0) {
            // System.out.println("split leaf page"+ leafPage.getId());
            leafPage = splitLeafPage(tid, dirtypages, leafPage, t.getField(keyField));
            // System.out.println("split leaf page: the page returned" + leafPage.getId());
        }
        // System.out.println("insert to "+"leafPage: " + leafPage.getId());
        // insert the tuple into the leaf page
        // System.out.println("insert tuple: " + t + " to leafPage: " + leafPage.getId());
    ...
    public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        // Update parent entry
        Field newKey = isRightSibling ? sibling.iterator().next().getField(keyField) : page.iterator().next().getField(keyField);
        entry.setKey(newKey);
        parent.updateEntry(entry);
        // System.out.println("after stealFromLeafPage ");
        // if (isRightSibling){
        //     System.out.println("stealing from right sibling");
        // }
        // else{
        //     System.out.println("stealing from left sibling");
        // }
        // Iterator<Tuple> ite = page.iterator();
        // while (ite.hasNext()) {
        //     System.out.println(ite.next());
        // }
        // Iterator<Tuple> ite2 = sibling.iterator();
        // while (ite2.hasNext()) {
    ...
    public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        BTreeEntry curr = null;
        int numToSteal = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;

        BTreeEntry newParentEntry = new BTreeEntry(parentEntry.getKey(), leftSibling.reverseIterator().next().getRightChild(), page.iterator().next().getLeftChild());
        page.insertEntry(newParentEntry);

        for (int i = 0; i < numToSteal-1; i++) {
            curr = siblingIterator.next();
            leftSibling.deleteKeyAndRightChild(curr);
    ...
    public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        BTreeEntry newParentEntry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), rightSibling.iterator().next().getLeftChild());
        page.insertEntry(newParentEntry);
        
        // steal the number we need - 1 because we need to move one up to the parent
        for (int i = 0; i < numToSteal-1; i++) {
            curr = siblingIterator.next();
            rightSibling.deleteKeyAndLeftChild(curr);
            page.insertEntry(curr);
        }
        // most the rightmost key in the right sibling to the parent, and then delete it from the right sibling
    ...
    public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        Iterator<Tuple> siblingIterator = rightPage.iterator();
        int numToSteal = rightPage.getNumTuples();
        for (int i = 0; i < numToSteal; i++) {
            Tuple tuple = siblingIterator.next();
    ...
    public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        BTreeEntry newParentEntry = new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(), rightPage.iterator().next().getLeftChild());
        leftPage.insertEntry(newParentEntry);

        Iterator<BTreeEntry> siblingIterator = rightPage.iterator();
        int numToSteal = rightPage.getNumEntries();
        for (int i = 0; i < numToSteal; i++) {
            BTreeEntry curr = siblingIterator.next();
